export const ROOT_OID = '1.2.246.562.10.00000000001';
export const BASE_PATH = '/koodisto-app';
export const HEALTH_CHECK_URI = '/koodisto-app/health';
export const API_LOKALISAATIO_PATH = '/lokalisointi/cxf/rest/v1/localisation';
export const API_BASE_PATH = '/koodisto-service/rest';
export const API_INTERNAL_PATH = '/koodisto-service/internal';
export const API_STATUS_PATH = '/koodisto-service/rest/session/maxinactiveinterval';
export const ORGANISAATIO_URI_LIST = [
    '1.2.246.562.10.00000000001',
    '1.2.246.562.10.2013112012294919827487',
    '1.2.246.562.10.78946234170',
    '1.2.246.562.10.12218193316',
    '1.2.246.562.10.91180885203',
];
